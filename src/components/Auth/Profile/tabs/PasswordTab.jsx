...
export default function PasswordTab() {
  ...
  const updatePassword = () => {
    if (currPass && nPass && conPass && nPass.length > 8 && conPass.length > 8) {
      if (auth()) {
        apiRequest
          .updatePass(
            {
          ...
          .catch((err) => {
            err.response.data &&
              toast.error(err.response.data.notification ? err.response.data.notification : ServeLangItem()?.Something_went_wrong);
          });
      } else {
        return false;
      }
      ...
      <div className="w-full flex lg:flex-row space-x-5 rtl:space-x-reverse lg:items-center">
        <div className="lg:w-[397px] w-full mb-10">
          <div className="input-field mb-6">
            ...
            >
              {ServeLangItem()?.Old_Password} <span className={"text-red-600"}>*</span>
            </label>
          ...
          <div className={`input-field mb-6 relative ${nPass && nPass.length < 8 ?"pt-4":"pt-0"}`}>
                     <InputCom
                        placeholder={"* * * * * *"}
            ...
            <div className="sm:flex sm:space-x-[30px] rtl:space-x-reverse items-center">
              <div className="w-[180px] h-[50px] lg:mb-0 mb-5">
                <button
                  ...
                  type="button"
                  className={`${currPass && nPass && conPass && nPass.length > 8 && conPass.length > 8 && nPass === conPass ?"yellow-btn ":"bg-qgray p-3"}`}
                  disabled={currPass.length < 0 && nPass.length < 8 && conPass.length < 8 && nPass !== conPass }
                >
