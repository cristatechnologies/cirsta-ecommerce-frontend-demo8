...
export default function OrderTab({ orders }) {
  ...
  const generatePDF = async (data) => {
    ...
    try {
        if (data) {
            const { order_address, order_products, total_amount, shipping_cost, order_id, coupon_coast } = data;
            let y = 20;
            ...
            pdf.setFont('Inter', '', 'normal');
            pdf.text(`${order_address.shipping_address} ${order_address.shipping_city} ${order_address.shipping_country}`, 45, y);
            y += 15;
            ...
            const tableData = order_products.map((item) => [item.product_name, item.qty, item.unit_price, (item.qty * item.unit_price).toFixed(2)]);
            ...
            const maxWidthProductName = tableData.reduce((max, item) => Math.max(max, pdf.getStringUnitWidth(item[0]) * fontSize), 0);
            const maxWidthQuantity = 30; // Maximum width for quantity column
            const actualPriceWidth = 50; // Width for the price column

            const remainingWidth = (pdf.internal.pageSize.width - startX - maxWidthQuantity - actualPriceWidth) / 2;
            const actualProductNameWidth = Math.min(maxWidthProductName, remainingWidth);

            console.log('actualProductNameWidth', actualProductNameWidth)
            // Draw table headers
            pdf.rect(startX, startY, actualProductNameWidth, cellHeight, 'F');
            pdf.rect(startX + actualProductNameWidth, startY, maxWidthQuantity, cellHeight, 'F');
            pdf.rect(startX + actualProductNameWidth + maxWidthQuantity, startY, actualPriceWidth, cellHeight, 'F');
            pdf.rect(startX + actualProductNameWidth + maxWidthQuantity + actualPriceWidth, startY, maxWidthQuantity, cellHeight, 'F');

            pdf.setTextColor(0, 0, 0); // Set text color for header
            pdf.text(tableHeaders[0], startX + 5, startY + 8);
            pdf.text(tableHeaders[1], startX + actualProductNameWidth + 5, startY + 8);
            pdf.text(tableHeaders[2], startX + actualProductNameWidth + maxWidthQuantity + 5, startY + 8);
            pdf.text(tableHeaders[3], startX + actualProductNameWidth + maxWidthQuantity + actualPriceWidth + 5, startY + 8);
            ...
            tableData.forEach((row) => {
                ...
                pdf.rect(startX, y, actualProductNameWidth, cellHeight + (actualProductNameWidth / 2));
                pdf.rect(startX + actualProductNameWidth, y, maxWidthQuantity, cellHeight + (actualProductNameWidth / 2));
                pdf.rect(startX + actualProductNameWidth + maxWidthQuantity, y, actualPriceWidth, cellHeight + (actualProductNameWidth / 2));
                pdf.rect(startX + actualProductNameWidth + maxWidthQuantity + actualPriceWidth, y, maxWidthQuantity, cellHeight + (actualProductNameWidth / 2));

                pdf.text(row[0], startX + 5, y + 8, { maxWidth: actualProductNameWidth - 10 });
                pdf.text(row[1].toString(), startX + actualProductNameWidth + 5, y + 8);
                pdf.text(`₹ ${row[2].toString()}`, startX + actualProductNameWidth + maxWidthQuantity + 5, y + 8);
                pdf.text(`₹ ${row[3].toString()}`, startX + actualProductNameWidth + maxWidthQuantity + actualPriceWidth + 5, y + 8);
            ...
            newTableData.forEach((row) => {
                ...
                pdf.text(row.col2.toString(), startX + actualProductNameWidth + 5, y + 8);
                pdf.text(row.col3.toString(), startX + actualProductNameWidth + maxWidthQuantity + 5, y + 8);
                pdf.text(row.col4.toString(), startX + actualProductNameWidth + maxWidthQuantity + actualPriceWidth + 5, y + 8);
                  ...
                  <td className="py-4 flex justify-center">
                    <div className="flex space-x-2 rtl:space-x-reverse items-center">
                      {/* <Link href={`/order/${item.order_id}`}>
                        ...
                        <div className="w-[116px] h-[46px] primary-bg text-qblack font-bold flex justify-center items-center cursor-pointer" onClick={()=>getOrderPdfDetails(item.order_id)}>
                          <span>Pdf bill</span>
                        </div>
                        ...
                        <a rel="noopener noreferrer">
                          <div className=" w-[126px] h-[46px] flex justify-center items-center cursor-pointer bg-[var(--primary-color)] ml-2">
                            <div className="flex rtl:space-x-reverse space-x-2 items-center">
                              <span className="text-sm font-bold text-qblack">
                                Dispute Order
                              </span>
                            </div>
                          </div>
